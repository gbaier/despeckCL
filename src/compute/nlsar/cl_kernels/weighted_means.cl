                              ...
                              const int patch_size)
{
    ...
    const int height_overlap_avg = height_overlap + patch_size - 1;
    const int width_overlap_avg  = width_overlap  + patch_size - 1;
            ...
            if ( (tx+x) < (BLOCK_SIZE+SEARCH_WINDOW_SIZE)
              ...
              && (out_y+y) < width_overlap ) {
                for(int d = 0; d<2*DIMENSION*DIMENSION; d++) {
                    covmat_local [d][tx+x][ty+y] = covmat_in[d*height_overlap*width_overlap \
                                                                  + (out_x+x)*width_overlap \
                                                                                + (out_y+y) ];
                }
            }
        }
    ...
    if (out_x < height_ori && out_y < width_ori) {
        ...
        for(int d = 0; d<2*DIMENSION*DIMENSION; d++) {
            covmat_out[d*height_overlap_avg*width_overlap_avg \
                        + (wsh+2*psh+out_x)*width_overlap_avg \
                                          + (wsh+2*psh+out_y)] = covmat_new[d];
        }
    }
}
