                              ...
                              const int patch_size)
{
    ...
    __local float covmat_local [DIMENSION][BLOCK_SIZE+SEARCH_WINDOW_SIZE][BLOCK_SIZE+SEARCH_WINDOW_SIZE];
            ...
            if ( (tx+x) < (BLOCK_SIZE+SEARCH_WINDOW_SIZE)
              ...
              && (out_y+y) < width_overlap ) {
                for(int d = 0; d<DIMENSION; d++) {
                    covmat_local [d][tx+x][ty+y] = covmat_in[d*height_overlap*width_overlap + (out_x+x)*width_overlap + (out_y+y) ];
                }
            }
        }
    }
                ...
                const float weight = weights[ out_x * width_ori * SEARCH_WINDOW_SIZE * SEARCH_WINDOW_SIZE \
                                                        + out_y * SEARCH_WINDOW_SIZE * SEARCH_WINDOW_SIZE \
                                                                                 + x * SEARCH_WINDOW_SIZE \
                                                                                                      + y ];
                float covmat_new[DIMENSION] = {0};
                for(int d = 0; d<DIMENSION; d++) {
                    covmat_new[d] += weight * covmat_local[d][tx+x][ty+y];
                }
                for(int d = 0; d<DIMENSION; d++) {
                    covmat_out[d*height_ori*width_ori + out_x*width_ori + out_y] += covmat_new[d];
                }
            }
        }
    }
