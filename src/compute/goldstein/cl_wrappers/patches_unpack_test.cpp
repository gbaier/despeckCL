...
TEST_CASE( "patches_unpack_rand", "[cl_kernels]" ) {

        // data setup
        const int height_packed = 24;
        const int width_packed  = 24;
        const int patch_size    = 32;
        const int overlap       = 4;
        ...
        for(int y = 0; y < height_unpacked; y++) {
            for(int x = 0; x < width_unpacked; x++) {
                const int xx = std::min(width_packed-1,  std::max(0, x - overlap));
                const int yy = std::min(height_packed-1, std::max(0, y - overlap));
        ...
        for(int y = 0; y < height_unpacked; y++) {
            for(int x = 0; x < width_unpacked; x++) {
                std::cout << std::setprecision(5) << (float) interf_real_unpacked[y*width_unpacked + x] << ":";
                std::cout << std::setprecision(5) << (float) interf_real_unpacked_desired[y*width_unpacked + x] << ", ";
            }
            std::cout << std::endl;
        }
...
TEST_CASE( "patches_unpack_rand_big", "[cl_kernels]" ) {
        ...
        for(int y = 0; y < height_unpacked; y++) {
            for(int x = 0; x < width_unpacked; x++) {
                ...
                const int rel_idx   = std::min(patch_size - 2*overlap - 1,  std::max(0, (x % patch_size) - overlap));
                const int patch_idy = y/patch_size;
                const int rel_idy   = std::min(patch_size - 2*overlap - 1,  std::max(0, (y % patch_size) - overlap));

                const int xx = patch_idx * (patch_size - 2*overlap) + rel_idx;
                const int yy = patch_idy * (patch_size - 2*overlap) + rel_idy;
